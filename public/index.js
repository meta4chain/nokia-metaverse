...
let goldPlayerPosition = [new THREE.Vector3(5.5, 0.35, -20.3), new THREE.Vector3(5.5, 1.5, -20.3), 0.35]
let silverPlayerPosition = [new THREE.Vector3(20, 3, -10), new THREE.Vector3(20, 4, -10), 0.35]
let silverPlayerRotation = [0, -79.5, 0]
let goldPlayerRotation = [0, 40, 0]
    ...
    loader.load(GLTFName, (gltf) => {
        ...
        if(collider){
            ...
            for(let j=0; j < gltf.scene.children.length; j++){
                if(gltf.scene.children[j].name !=  "Puffs" && gltf.scene.children[j].name !=  "Cadeiras_Mesinhas" && gltf.scene.children[j].name !=  "Bancos" && gltf.scene.children[j].name !=  "Racks_Tv" && gltf.scene.children[j].name !=  "Parede_Exterior"){
...
function onStart(level) {
    ...
    manager.onLoad = (url, itemsLoad, itemsTotal) => {
        ...
        let doc = document.getElementById("box-overlay")
        // console.log(doc)
...
function createScene(level, loader) {
    ...
    if (level == "gold") {
        let collider = true;
        loaderGLTF(loader, "Ouro02/Ouro02.glb", { x: 0, y: 2, z: 0 }, 1, collider)
    }
